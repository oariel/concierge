...
// Find a user bu Id
function findUser(connector, message, conv_id, id) {
    return new Promise(resolve => {
        connector.fetchMemberList(
            message.address.serviceUrl,
            conv_id,
            teams.TeamsMessage.getTenantId(message),
            function (err, result) {
                if (err) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        if (result[i].id == id) {
                            if (result[i].email)
                                ...
                                resolve({ email: "", surname: "", givenName: "" });
                        }
                    }
                }
            });
...
var bot = new builder.UniversalBot(connector, async function (session) {
    ...
    var userInfo = await findUser(connector, session.message, conv_id, user_id);
